export default function PreFooterMain2() {
    return (
        <div className="px-4 my-16">
            <div className="max-w-6xl mx-auto space-y-8">
                {/* Corbett Roar Section */}
                <div className="rounded-3xl p-8 md:p-12" style={{ backgroundColor: '#F4F0F070' }}>
                    <h1 className="text-4xl font-serif text-white mb-6">Corbett Roar</h1>
                    <p className="text-white text-lg">
                        Corbett Roar Safari, your trusted partner for the best Jim Corbett packages! 
                        We specialize in creating unforgettable experiences for clients from around the world. 
                        Whether you're looking to book resorts, safaris, car rentals, or complete Corbett tour packages, 
                        we have you covered. Our team of experts provides personalized guidance to help you choose 
                        the perfect package—be it budget-friendly, mid-range, or luxurious. Plan your dream Jim Corbett 
                        adventure with Corbett Roar Safari today!
                    </p>
                </div>

                {/* Contact Us Section */}
                <div className="rounded-3xl p-8 md:p-12" style={{ backgroundColor: '#F4F0F070' }}>
                    <h1 className="text-4xl font-serif text-white mb-6">Contact Us Now</h1>
                    <div className="text-white space-y-4">
                        <p>Ayaan - 6396021400 , Arif - 9837578586</p>
                        <p>corbettroar@gmail.com</p>
                        <p>Near lakhanpur chungi, aamdanda road, ramnagar(Nainital), 244715</p>
                    </div>
                </div>
            </div>
        </div>
    )
}